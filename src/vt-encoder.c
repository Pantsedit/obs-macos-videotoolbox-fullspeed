/*
 * VideoToolbox Full Speed for OBS Studio
 *
 * Apple VideoToolbox H.264/HEVC hardware encoders tuned for running several
 * high-resolution outputs at once (e.g. 2x 4K60 HEVC on an M2 Max).
 *
 * Based on the mac-videotoolbox plugin from OBS Studio
 * (https://github.com/obsproject/obs-studio), licensed under GPL-2.0-or-later.
 * Changes: real-time encoding on by default, expected frame rate hint off by
 * default, optional encode timing log, separate encoder IDs so the built-in
 * encoders stay untouched.
 *
...
#define VT_LOG(level, format, ...) blog(level, "[VT Full Speed]: " format, ##__VA_ARGS__)
#define VT_LOG_ENCODER(encoder, codec_type, level, format, ...)                        \
	blog(level, "[VT Full Speed %s: '%s']: " format, obs_encoder_get_name(encoder), \
...
#ifdef __aarch64__
static bool is_apple_silicon = true;
#else
static bool is_apple_silicon = false;
#endif
...
static void sample_encoded_callback(void *data, void *source, OSStatus status, VTEncodeInfoFlags info_flags,
...
static OSStatus create_encoder(struct vt_encoder *enc)
{
	...
	if (enc->codec_type == kCMVideoCodecType_H264 || enc->codec_type == kCMVideoCodecType_HEVC) {
		...
		CFTypeRef session_values[4] = {MaxKeyFrameInterval, AllowFrameReordering, ProfileLevel,
					       ExpectedFrameRate};

		// With an expected frame rate set, high resolution/frame rate sessions may be spread
		// across multiple encode engines, starving any other concurrent session.
		CFIndex num_session_properties = enc->frame_rate_hint ? 4 : 3;

		CFDictionaryRef session_properties =
			CFDictionaryCreate(kCFAllocatorDefault, session_keys, session_values, num_session_properties,
...
static void dump_encoder_info(struct vt_encoder *enc)
{
	VT_BLOG(LOG_INFO,
		...
		enc->realtime ? "on" : "off", enc->frame_rate_hint ? "off" : "on",
...
static bool update_params(struct vt_encoder *enc, obs_data_t *settings)
{
	...
	// "Optimize for multiple outputs": real-time on and no expected frame rate hint.
	// Both are needed; either alone does not help. The individual keys can still be set
	// in the encoder JSON for testing but are not shown in the UI.
	bool multi_output = obs_data_get_bool(settings, "multi_output");
	enc->realtime = obs_data_has_user_value(settings, "realtime") ? obs_data_get_bool(settings, "realtime")
								      : multi_output;
	enc->log_timing = obs_data_get_bool(settings, "log_timing");
	enc->frame_rate_hint = obs_data_has_user_value(settings, "frame_rate_hint")
				       ? obs_data_get_bool(settings, "frame_rate_hint")
				       : !multi_output;
...
static bool get_cached_pixel_buffer(struct vt_encoder *enc, CVPixelBufferRef *buf)
{
	OSStatus code;
...
#define TIMING_LOG_INTERVAL_NS 10000000000ULL

static void update_timing(struct vt_encoder *enc, uint64_t copy_start, uint64_t encode_start, uint64_t encode_end)
{
	uint64_t copy_time = encode_start - copy_start;
	uint64_t encode_time = encode_end - encode_start;

	if (!enc->timing.period_start)
		enc->timing.period_start = copy_start;

	enc->timing.frames++;
	enc->timing.copy_total += copy_time;
	enc->timing.encode_total += encode_time;
	if (copy_time > enc->timing.copy_max)
		enc->timing.copy_max = copy_time;
	if (encode_time > enc->timing.encode_max)
		enc->timing.encode_max = encode_time;

	if (encode_end - enc->timing.period_start < TIMING_LOG_INTERVAL_NS)
		return;

	double frames = (double)enc->timing.frames;
	VT_BLOG(LOG_INFO,
		"timing: %llu frames, copy avg %.3f ms / max %.3f ms, "
		"EncodeFrame avg %.3f ms / max %.3f ms",
		(unsigned long long)enc->timing.frames, (double)enc->timing.copy_total / frames / 1000000.0,
		(double)enc->timing.copy_max / 1000000.0, (double)enc->timing.encode_total / frames / 1000000.0,
		(double)enc->timing.encode_max / 1000000.0);

	memset(&enc->timing, 0, sizeof(enc->timing));
	enc->timing.period_start = encode_end;
}
...
static bool vt_encode(void *data, struct encoder_frame *frame, struct encoder_packet *packet, bool *received_packet)
{
	...
	uint64_t copy_start = enc->log_timing ? os_gettime_ns() : 0;
	...
	uint64_t encode_start = enc->log_timing ? os_gettime_ns() : 0;
	...
	if (enc->log_timing)
		update_timing(enc, copy_start, encode_start, os_gettime_ns());
...
static const char *vt_getname(void *data)
{
	...
	if (type_data->codec_type == kCMVideoCodecType_HEVC)
		return obs_module_text("VTHEVCEncFullSpeed");
	return obs_module_text("VTH264EncFullSpeed");
}
...
static obs_properties_t *vt_properties_h26x(void *data __unused, void *type_data)
{
	...
	p = obs_properties_add_bool(props, "multi_output", obs_module_text("MultiOutput"));
	obs_property_set_long_description(p, obs_module_text("MultiOutput.ToolTip"));

	p = obs_properties_add_bool(props, "log_timing", obs_module_text("LogTiming"));
	obs_property_set_long_description(p, obs_module_text("LogTiming.ToolTip"));
...
static void vt_defaults(obs_data_t *settings, void *data)
{
	...
	obs_data_set_default_bool(settings, "log_timing", false);
	obs_data_set_default_bool(settings, "multi_output", true);
...
{
	return "Apple VideoToolbox H.264/HEVC encoders tuned for multiple simultaneous high resolution outputs";
}
static dispatch_group_t encoder_list_dispatch_group;
static CFArrayRef encoder_list;
...
void obs_module_post_load(void)
{
...
#ifdef OBS_ENCODER_CAP_MULTITRACK_DYN_BITRATE
	...
	for (CFIndex i = 0; i < size; i++) {
		...
		CFBooleanRef is_hw = CFDictionaryGetValue(encoder_dict, kVTVideoEncoderList_IsHardwareAccelerated);
		if (!is_hw || !CFBooleanGetValue(is_hw))
			continue;
		...
		case kCMVideoCodecType_H264:
			...
			info.codec = "h264";
			obs_id = "vt_fullspeed_h264";
			break;
		case kCMVideoCodecType_HEVC:
			info.get_properties2 = vt_properties_h26x;
			info.codec = "hevc";
			obs_id = "vt_fullspeed_hevc";
			break;
		default:
			continue;
		}
		...
		// Register each codec once (first hardware encoder wins)
		bool already = false;
		for (size_t k = 0; k < registered_count; k++)
			already |= strcmp(registered[k], obs_id) == 0;
		if (already) {
			bfree(id);
			bfree(disp_name);
			continue;
		}
		registered[registered_count++] = obs_id;
	...
	VT_LOG(LOG_INFO, "Loaded version %s: registered %zu encoder(s)", PLUGIN_VERSION, registered_count);
}
